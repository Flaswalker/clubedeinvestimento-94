  ...
  return <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "glass-panel py-3 backdrop-blur-xl" : "py-5"}`}>
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <Link to="/" className="flex items-center space-x-2 transition hover:opacity-80">
              ...
              <span className="font-bold text-xl tracking-tight">Clube de Investimento</span>
            </Link>
          </div>
          ...
          <div className="md:hidden flex items-center">
            <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="inline-flex items-center justify-center p-2 rounded-md focus:outline-none" aria-expanded="false">
      ...
      {isMobileMenuOpen && <div className="md:hidden glass-panel animate-fade-in">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            {user ? <>
                <Link to={user.isAdmin ? "/admin" : "/dashboard"} className="block px-3 py-2 text-base font-medium transition-all hover:text-primary" onClick={() => setIsMobileMenuOpen(false)}>
                  {user.isAdmin ? "Painel Admin" : "Minha Conta"}
                </Link>
                <button onClick={() => {
            logout();
            setIsMobileMenuOpen(false);
                ...
                <Link to="/login" className="block px-3 py-2 text-base font-medium transition-all hover:text-primary" onClick={() => setIsMobileMenuOpen(false)}>
                  Entrar
                </Link>
                <Link to="/register" className="block px-3 py-2 text-base font-medium transition-all hover:text-primary" onClick={() => setIsMobileMenuOpen(false)}>
                  Cadastrar
                </Link>
              </>}
          </div>
